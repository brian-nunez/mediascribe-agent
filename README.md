...
- `research_mediascribe(primary_query, related_queries, max_articles, language)`: runs multiple Mediascribe searches, deduplicates results, and fetches full articles from `/api/public/blogs/{blog_id}`.
...
- Broad architecture questions should use 5 to 8 query angles and fetch 8 to 10 full articles.
...
- Research selects at least one top unique article per query before filling remaining slots by score.
